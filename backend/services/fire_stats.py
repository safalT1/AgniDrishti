...
def get_confidence_level_counts():
    ...
    # Group confidence into three standard scientific bins: Low, Moderate, High
    low_count = int((df['confidence'] < 60).sum())
    mod_count = int(((df['confidence'] >= 60) & (df['confidence'] < 85)).sum())
    high_count = int((df['confidence'] >= 85).sum())

    return [
        {"confidence": "Low (<60%)", "count": low_count},
        {"confidence": "Moderate (60-85%)", "count": mod_count},
        {"confidence": "High (>=85%)", "count": high_count}
    ]
