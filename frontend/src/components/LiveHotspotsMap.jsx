...
import { fetchLiveFires } from '../services/fireApi';
...
export default function LiveHotspotsMap({ sensor = 'MODIS_NRT', days = 1 }) {
  ...
  // Fetch fire points (using backend proxy to bypass CORS errors)
  useEffect(() => {
    setLoading(true);
    setError(null);
    
    fetchLiveFires(sensor, days)
      .then(res => {
        // res contains { fires: [...], count: ... }
        setFires(res.fires || []);
        setLoading(false);
      })
      .catch(err => {
        console.error('Failed to fetch live fires via backend proxy', err);
